          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd changepirce="0" :class="{ active: item2.isChecked == 1 }" v-for="item2 in item.spuSaleAttrValueList" :key="item2.id" @click="changeActive(item.spuSaleAttrValueList, item2)">{{ item2.saleAttrValueName }}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model.trim="skuNum" @change="changeNum" />
...
export default {
  ...
  methods: {
    // 产品售卖属性高亮
    // arr每一项的数组。item是数组的每一项
    changeActive(arr, item) {
      // 先干掉所有人，遍历数组把所有项的ischecked变为0
      arr.forEach((item) => {
        item.isChecked = 0
      })
      // 留下我自己，把点击的一项的isChecked变为1
      item.isChecked = 1
    },
    sub() {
      if (this.skuNum > 1) {
    ...
    async addShopCart() {
      try {
        await this.$store.dispatch('Detail/getShopCart', { skuId: this.$route.params.skuId, skuNum: this.skuNum })
