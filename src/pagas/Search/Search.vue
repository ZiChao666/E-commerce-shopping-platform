    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="details clearfix">
          ...
          <!-- 分页 -->
          <Pagination :pageNo="searchParams.pageNo" :pageSize="searchParams.pageSize" :total="total" :continues="5" @sendPageNo="getPageNo"></Pagination>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    // 删除分类的名字
    removecategoryName() {
      // 把数据置空了，还需要向服务器发请求，但是置空的数据还是会以空字符串带给服务器，可以把空字符串改为undefined这样空字符串就不会带给服务器了
      this.searchParams.category1Id = undefined
      this.searchParams.category2Id = undefined
      this.searchParams.category3Id = undefined
      this.searchParams.categoryName = undefined
      // this.getData()
      // ×了之后跳转到默认的search界面,路由地址也要跳
      if (this.$route.params) {
        // 这里要严谨一下，本质上我们是要删除query的，但是如果路径当中出现params，那不应该删除，只删除query
    ...
    attrInfos(attrs, attrs2) {
      // 整理商品属性的数据

      let property = `${attrs.attrId}:${attrs2}:${attrs.attrName}`
      // 判断去重
