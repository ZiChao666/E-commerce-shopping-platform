<template>
  <div class="trade-container">
    ...
    <div class="content" v-if="userAddressList">
      ...
      <div class="bbs">
        <h5>买家留言：</h5>
        <textarea placeholder="建议留言前先与商家沟通确认" class="remarks-cont" v-model.trim="message"></textarea>
      </div>
      <div class="line"></div>
      <div class="bill">
        <h5>发票信息：</h5>
...
export default {
  ...
  computed: {
    ...
    // 总价
    totalPrice() {
      return this.detailArrayList.reduce((pre, item) => (pre += item.skuNum * item.orderPrice), 0)
    },
    // 总的件数
    sum() {
      return this.detailArrayList.reduce((pre, item) => (pre += item.skuNum), 0)
    }
  },
  ...
  methods: {
    ...
    async submitOrder() {
      // 交易编码
      let tradeNo = this.$store.state.Trade.ordersInfo.data.tradeNo
      // 其他的数据
      let data = {
        // consignee: this.finalAddress.consignee,
        // consigneeTel: this.finalAddress.phoneNum,
        // deliveryAddress: this.finalAddress.userAddress,
        consignee: '帅比',
        consigneeTel: '1314520',
        deliveryAddress: '你心里',
        paymentWay: 'ONLINE',
      ...
      if (result.code == 201) {
        ...
        this.$router.push('/pay?orderId=' + 123456)
      }
    }
  }
}
